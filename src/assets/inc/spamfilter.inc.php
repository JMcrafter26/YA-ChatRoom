...
class SpamFilter
{
	...
	public function check_text_and_blacklist($text)
	{
		foreach ($this->blacklists as $blacklist_filename)
		{
			$match = $this->regex_match_from_blacklist($text, $blacklist_filename);
			if ($match) return  array($match, str_replace('.txt', '', str_replace('blacklist-', '', $blacklist_filename)));
		}

		$repeat = $this->check_repeating_text($text);
		if ($repeat) return array($text, 'keyboardsmash');
	}

	public function check_repeating_text($text) {
		// if text is repeating, e.g. "asdfasdf" or "lkjhlkjhkljh",... it's spam
		
		// text must be over 4 characters long
		if (strlen($text) < 5) return false;

		// text must be repeated at least 2 times, e.g. "asdfasdf" -> "asdf" is repeated
		$half_length = strlen($text) / 2;
		for ($i = 0; $i < $half_length; $i++)
		{
			$substring = substr($text, $i, $half_length);
			$repeated = substr_count($text, $substring);
			if ($repeated > 1) return true;
		}

		if (strlen($text) < 6) return false;

		$third_length = strlen($text) / 3;
		for ($i = 0; $i < $third_length; $i++)
		{
			$substring = substr($text, $i, $third_length);
			$repeated = substr_count($text, $substring);
			if ($repeated > 1) return true;
		}
		...
		$quarter_length = strlen($text) / 4;
		for ($i = 0; $i < $quarter_length; $i++)
		{
			$substring = substr($text, $i, $quarter_length);
			$repeated = substr_count($text, $substring);
			if ($repeated > 1) return true;
		}
